...
type
  TfrmTomaMuestras1 = class(TfrmCustomModule)
    ...
    tbPrueba: TADOTable;
    tbPruebaPRUEBAID: TStringField;
    tbPruebaDEPARTAMENTO: TStringField;
    tbPruebaALIAS: TStringField;
    qrLabel: TADOQuery;
    ...
    tbDepartamentoCodigo: TIntegerField;
    tbDepartamentoDepartamento: TWideStringField;
    tbDepartamentoDptoId: TStringField;
...
var
  frmTomaMuestras1       : TfrmTomaMuestras1;
...
uses ActionsDM, cxExportGrid4Link, DataModule, Main, DataBanco, BancoReportes;

{$R *.dfm}


//Buscando el Label de la Prueba Registrada.
function TfrmTomaMuestras1.Buscar_Labels(muestra: String; prueba: String): Boolean;
Var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
procedure TfrmTomaMuestras1.CrearLabels(SecActual: Integer);
var
...
procedure TfrmTomaMuestras1.InsertarLabel;
var
 Valor      : String;
 Qinsert    : TADOQuery;
 labelrecid : Integer;
...
function TfrmTomaMuestras1.ExistenPruebasProcesar : Boolean;
var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
procedure TfrmTomaMuestras1.ActPruebas(_flebotomista : string; fecha : TDateTime; hora : string);
begin
  DM.qrParametro.Close;
  DM.qrParametro.Open;
{
...
procedure TfrmTomaMuestras1.BtFacturaClick(Sender: TObject);
begin
//  frmMain.LanzaVentana(-7995);
  recid:= DMB.BuscarDonacion;
  BuscarDatos;
end;

procedure TfrmTomaMuestras1.BtNoProcesarClick(Sender: TObject);
begin
  No_Procesar;
end;

procedure TfrmTomaMuestras1.BtPendienteClick(Sender: TObject);
begin
  VerPendiente;
end;

procedure TfrmTomaMuestras1.BtProcesarClick(Sender: TObject);
begin
...
procedure TfrmTomaMuestras1.BtUrgenteClick(Sender: TObject);
begin
  MarcarUrgente;
end;


procedure TfrmTomaMuestras1.BtImprimirClick(Sender: TObject);
begin
 inherited;

  if qrEntradaPacienteDetalle.Eof then exit;

  qrEntradaPacienteDetalle.First;

  tbPrueba.Close;
  tbPrueba.Open;
  tbDepartamento.Close;
  tbDepartamento.Open;
  tbDonacion.Close;
  tbDonacion.Open;
  ...
  while not qrEntradaPacienteDetalle.Eof do
    begin
      qrLabel.Close;
      qrlabel.Parameters.ParamByName('MuestraNo').Value:= trim(qrEntradaPacienteMuestraNoAS.Value);
      qrlabel.Parameters.ParamByName('DeptId').Value:= trim(tbPruebaDepartamento.Value);
      qrlabel.Open;

      if qrlabel.Eof then
        begin
          qrlabel.Insert;
          qrLabelMuestraNo.Value:= qrEntradaPacienteMuestraNoAS.Value;
          qrlabelDeptId.Value:= tbPruebaDepartamento.Value;
        end
      else
        qrLabel.Edit;

      if (pos(trim(tbPruebaAlias.Value), qrlabelPruebas.Value) = 0)  then
        qrlabelPruebas.Value:= qrlabelPruebas.Value + trim(tbPruebaAlias.Value) + ' ';

      qrLabelDepartamento.Value:= tbDepartamentoDepartamento.Value;
      qrLabelNombre.Value:= qrEntradaPacienteNOMBREPACIENTE.Value;
  ...
  BSReportes.EtiquetaPequena(qrEntradaPacienteMuestraNoAS.Value);
...
procedure TfrmTomaMuestras1.BtLabelUnicoClick(Sender: TObject);
begin
...
procedure TfrmTomaMuestras1.FormCreate(Sender: TObject);
begin
  inherited;
...
//  doctor.Color := $0005EAD9;
  ...
  recid:= DMB.PruebaActual;
  BuscarDatos;

end;


procedure TfrmTomaMuestras1.FormDestroy(Sender: TObject);
begin
  inherited;
...
procedure TfrmTomaMuestras1.FormPaint(Sender: TObject);
begin
  inherited;
...
procedure TfrmTomaMuestras1.qrEntradaPacienteDetalleCalcFields(
  DataSet: TDataSet);
var
...
procedure TfrmTomaMuestras1.qrEntradaPacienteDetalleSeleccionarValidate(
  Sender: TField);
begin
  inherited;
//  VerPendiente;
...
procedure TfrmTomaMuestras1.qrMaterialesFacturaCantidadValidate(Sender: TField);
begin
  inherited;
...
procedure TfrmTomaMuestras1.SetColorTo(iColor: TColor);
var
  i : Integer;
  oColor : TColor;
begin
...
procedure TfrmTomaMuestras1.BuscarDatos;
begin
  Postear;
...
procedure TfrmTomaMuestras1.BuscarMateriales(registro : Integer);
begin
...
function TfrmTomaMuestras1.BuscarDia(dia: Integer) : Integer;
Var
 valor : Integer;
begin
 valor := 0;
...
function TfrmTomaMuestras1.FechaPrometida(Prueba: string) : TDate;
Var
 fecha : TDate;
 qtest : TADOQuery;
 qfrec : TADOQuery;
...
procedure TfrmTomaMuestras1.MarcarUrgente;
Var
 busq : String;
 qact : TADOQuery;
begin
...
procedure TfrmTomaMuestras1.VerPendiente;
Var
 qact : TADOQuery;
begin
...
procedure TfrmTomaMuestras1.No_Procesar;
Var
 qact : TADOQuery;
begin
...
procedure TfrmTomaMuestras1.RefrescarData;
Var
  Bm : TBookMark;
begin
...
procedure TfrmTomaMuestras1.cxGrid1DBTableView1StylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; out AStyle: TcxStyle);
begin
  inherited;
...
procedure TfrmTomaMuestras1.FindMuestra;
Var
 qfind  : TADOQuery;
begin
...
procedure TfrmTomaMuestras1.EdMuestraDblClick(Sender: TObject);
begin
...
Procedure TfrmTomaMuestras1.VerCaption;
begin
  If (qrEntradaPacienteResultadoDoctor.Value = 1) Then
    Begin
...
procedure TfrmTomaMuestras1.Act_Fecha;
begin
  fecha_label := Date;
...
procedure TfrmTomaMuestras1.InsertarMaterial;
Var
...
function TfrmTomaMuestras1.Buscar_Linea: Integer;
Var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
procedure TfrmTomaMuestras1.Postear;
begin
// If ( qrEntradaPacienteDetalle.RecordCount > 0) Then
...
procedure TfrmTomaMuestras1.Insertar_Impresion(_usuario : string; _fecha : TDateTime; _hora : string);
var
 Valor      : String;
 qfind      : TADOQuery;
 Qinsert    : TADOQuery;
...
procedure TfrmTomaMuestras1.Insertar_Impresiones(_usuario : string; _fecha : TDateTime; _hora : string);
var
 Valor      : String;
 qfind      : TADOQuery;
 Qinsert    : TADOQuery;
...
procedure TfrmTomaMuestras1.ImprimirLabel;
var
 qfind    : TADOQuery;
 texto    : TextFile;
 labels   : Integer;
...
procedure TfrmTomaMuestras1.ImprimirUnico;
var
 qfind  : TADOQuery;
 texto  : TextFile;
 labels : Integer;
