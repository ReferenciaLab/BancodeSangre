...
type
  TfrmPuntoVenta = class(TfrmCustomModule)
    ...
    qrEntradaPacienteOrigen: TIntegerField;
    ...
    procedure qrEntradaPacienteOrigenValidate(Sender: TField);
  ...
  protected
    ...
    procedure CrearTransCliente(cliente,tipodoc,documento,tipoaplica,documentoaplica : String;fecha : TDateTime;valor : Double);
...
tipoaplica: string; documentoaplica: string;fecha : TDateTime; valor: Double);
var
 qinsert : TADOQuery;
begin
 qinsert := DM.NewQuery;
 ...
 qinsert.Close;
 qinsert.SQL.Text := 'Insert into PTClienteTransacciones(ClienteId,fecha,TipoDoc,Documento,TipoAplica,DocumentoAplica,Valor) '+
...
// GridPruebasEntradasPacienteDetCoberturaEspecial.Visible := false;
 case qrEntradaPacienteOrigen.Value of
  0,3 : lcGeneralItem15.Caption := 'Referencia';
    1 : begin
...
begin
  ...
  if DM.qrParametroIdentificadorMuestra.Value <> '' then
    result := DM.qrParametroIdentificadorMuestra.Value + '-' +
              formatfloat('00',strtofloat(DM.cursucursal))+ '-' +
              formatfloat('000000',DM.qrParametroSecuenciaMuestra.AsFloat)
  else
    result := formatfloat('00',strtofloat(DM.cursucursal))+ '-' +
              formatfloat('000000',DM.qrParametroSecuenciaMuestra.AsFloat);

  DM.qrParametro.Edit;
  DM.qrParametroSecuenciaMuestra.Value := DM.qrParametroSecuenciaMuestra.Value + 1;
  DM.qrParametro.Post;
end;
...
begin
  ...
  if DM.qrParametroIdentificadorRecibo.Value <> '' then
    result := DM.qrParametroIdentificadorRecibo.Value + '-' +
              formatfloat('00',strtofloat(DM.cursucursal))+ '-' +
              formatfloat('000000',DM.qrParametroSecuenciaRecibo.AsFloat)
  else
    result := formatfloat('00',strtofloat(DM.cursucursal))+ '-' +
              formatfloat('000000',DM.qrParametroSecuenciaRecibo.AsFloat);

  DM.qrParametro.Edit;
  DM.qrParametroSecuenciaRecibo.Value := DM.qrParametroSecuenciaRecibo.Value + 1;
  DM.qrParametro.Post;
end;
...
begin
  ...
  if DM.qrParametroIdentificadorNC.Value <> '' then
    result := DM.qrParametroIdentificadorNC.Value + '-' +
              formatfloat('00',strtofloat(DM.cursucursal))+ '-' +
              formatfloat('000000',DM.qrParametroSecuenciaNC.AsFloat)
  else
    result := formatfloat('00',strtofloat(DM.cursucursal))+ '-' +
              formatfloat('000000',DM.qrParametroSecuenciaNC.AsFloat);

  DM.qrParametro.Edit;
  DM.qrParametroSecuenciaNC.Value := DM.qrParametroSecuenciaNC.Value + 1;
  DM.qrParametro.Post;
end;
...
begin
   ...
   DescuentoPorc := (qrEntradaPacienteDetallePrecio.Value - qrEntradaPacienteDetalleDescuento.Value) * (qrEntradaPacienteDetalleDescPct.Value / 100);
   ...
   if (qrEntradaPacienteDescuentoPorc.Value  > 0) then
       DescuentoPorc := DescuentoPorc + ((qrEntradaPacienteDetalleTotalLinea.Value - DescuentoPesosCab) * qrEntradaPacienteDescuentoPorc.Value /100);
...
begin
 ...
 begin
  ...
  begin
   ...
   if not qrEntradaPacienteCoberturaExpPorc.Value then
   begin
   ...
   begin
    if (qrEntradaPacienteCoberturaValor.Value  > 0) then
        coberturaporc := (qrEntradaPacienteDetalleTotalLinea.Value - CoberturaPesosCab) * qrEntradaPacienteCoberturaValor.Value /100;
   end;
...
procedure TfrmPuntoVenta.btcobrarClick(Sender: TObject);
begin
  inherited;
...
begin
 ...
 begin
  ...
  begin
   ...
   qrEntradaPacienteFecha.Value := Date;
...
begin
  inherited;
  if (key = vk_f2) then btpacienteClick(nil);
  if (key = vk_f3) then btpruebaClick(nil);
  if (key = vk_f4) then btclienteClick(nil);
  if (key = vk_f5) then bteliminarClick(nil);
  if (key = vk_f6) then btcancelarClick(nil);
  if (key = vk_f7) then btgrabarClick(nil);
  if (key = vk_f8) then btcobrarClick(nil);
  if (key = vk_f9) then btnotacreditoClick(nil);
end;
...
procedure TfrmPuntoVenta.ppSummaryBand1BeforePrint(Sender: TObject);
begin
  inherited;
...
procedure TfrmPuntoVenta.qrEntradaPacienteDetallePruebaIDValidate(
  Sender: TField);
begin
  inherited;
...
procedure TfrmPuntoVenta.qrEntradaPacienteOrigenValidate(Sender: TField);
var
 qfind : TADOQuery;
begin
  inherited;
  ...
  begin
   ...
   if qfind.FieldByName('Origen').AsInteger <>  qrEntradaPacienteOrigen.Value then
      raise exception.CreateFmt('El cliente seleccionado no pertenece a este tipo de cliente.',[]);
...
begin
 ...
 qrEntradaPacienteNeto.Value := qrEntradaPacienteSubTotal.Value - qrEntradaPacienteCoberturaSeguro.Value;
...
begin
 qupdate := DM.NewQuery;
 qupdate.Close;
 qupdate.SQL.Text := 'Update PTCobroDetalle Set Monto = :mont where Recid = :rec';
 ...
 begin
  ...
  qupdate.Parameters[1].Value := mdPagorefrecid.Value;
  qupdate.ExecSQL;
...
begin
 ...
 case qrEntradaPacienteTipoDocumento.Value of
   0 : begin
        if DM.qrParametroIndetificadorFactura.Value <> '' then
           result := DM.qrParametroIndetificadorFactura.Value + '-'+ formatfloat('00',strtofloat(DM.cursucursal))+'-'+formatfloat('000000',DM.qrParametroSecuenciaEntrada.AsFloat)
        else
           result := formatfloat('00',strtofloat(DM.cursucursal))+'-'+formatfloat('000000',DM.qrParametroSecuenciaEntrada.AsFloat);

        DM.qrParametro.Edit;
        DM.qrParametroSecuenciaEntrada.Value := DM.qrParametroSecuenciaEntrada.Value + 1;
        DM.qrParametro.Post;
       end;
   1 : begin
        if DM.qrParametroIdentificadorCotizacion.Value <> '' then
           result := DM.qrParametroIdentificadorCotizacion.Value + '-'+ formatfloat('00',strtofloat(DM.cursucursal))+'-'+formatfloat('000000',DM.qrParametroSecuenciaCot.AsFloat)
        else
           result := formatfloat('00',strtofloat(DM.cursucursal))+'-'+formatfloat('000000',DM.qrParametroSecuenciaCot.AsFloat);
        DM.qrParametro.Edit;
        DM.qrParametroSecuenciaCot.Value := DM.qrParametroSecuenciaCot.Value + 1;
        DM.qrParametro.Post;
       end;

   2 : begin
        if DM.qrParametroIdentificadorDevolucion.Value <> '' then
           result := DM.qrParametroIdentificadorDevolucion.Value + '-'+ formatfloat('00',strtofloat(DM.cursucursal))+'-'+formatfloat('000000',DM.qrParametroSecuenciaDev.AsFloat)
        else
           result := formatfloat('00',strtofloat(DM.cursucursal))+'-'+formatfloat('000000',DM.qrParametroSecuenciaDev.AsFloat);           
        DM.qrParametro.Edit;
        DM.qrParametroSecuenciaDev.Value := DM.qrParametroSecuenciaDev.Value + 1;
        DM.qrParametro.Post;
      end;
 end;
...
begin
 ...
 qinsert.Parameters[2].Value := Date;
 ...
 qinsert.Close;
 qinsert.SQL.Clear;
 qinsert.SQL.Text := 'insert into PTCobroDetalle(CobroID,FormaDePagoID,Monto,Orden,RefRecid) '+
                     '            Values(:0,:1,:2,:3,:4)';
  ...
  // CambiarLaboratorioNoenlasLineas1.Enabled := false;
   ...
   // CambiarLaboratorioNoenlasLineas1.Enabled := CanCambiarLaboratorio;
    ...
    // edcoberturaporc.Properties.ReadOnly := qrEntradaPacienteCoberturaConfirmada.Value;
     ...
     edcoberturavalor.Properties.ReadOnly := qrEntradaPacienteCoberturaConfirmada.Value;
    // CambiarLaboratorioNoenlasLineas1.Enabled := CanCambiarLaboratorio;
     if qrEntradaPacienteOrigen.Value = 1 then
        AsignarDescuentoPermitidoalUsuario1.Enabled := false
     else
        AsignarDescuentoPermitidoalUsuario1.Enabled := true;
     EliminarDescuentoAplicado1.Enabled := True;        
