...
type
  TfrmEntradaLaboratorio = class(TfrmCustomModule)
    ...
    qrDatosProductoProductoID: TWideStringField;
    qrDatosProductoProductoDes: TWideStringField;
    qrDatosProductoPrecio: TBCDField;
    qrDatosProductoPrecioDolares: TBCDField;
    qrDatosProductoDias: TIntegerField;
    qrDatosProductoCosto: TBCDField;
    ...
    procedure qrEntradaPacienteDetalleNewRecord(DataSet: TDataSet);
...
var
  frmEntradaLaboratorio   : TfrmEntradaLaboratorio;
...
uses ActionsDM, DataModule, Main, Pymeconst, DataBanco;

{$R *.dfm}


function TfrmEntradaLaboratorio.ValidateCliente : Boolean;
var
 qfind : TADOQuery;
 ret : Boolean;
 suc : Boolean;
...
procedure TfrmEntradaLaboratorio.CrearTransCliente(cliente: string; tipodoc: string; documento: string;
tipoaplica: string; documentoaplica: string; fecha : TDateTime; valor: Double; moneda :string);
var
 qinsert : TADOQuery;
begin
...
procedure TfrmEntradaLaboratorio.CuadredeCaja1Click(Sender: TObject);
begin
  inherited;
  frmMain.LanzaVentana(1402);
end;

procedure TfrmEntradaLaboratorio.CuadredeCaja2Click(Sender: TObject);
begin
  inherited;
  frmMain.LanzaVentana(-7987);
end;

procedure TfrmEntradaLaboratorio.RefrescarGrid(SoloLectura: Boolean);
begin
...
procedure TfrmEntradaLaboratorio.ActInterface;
begin
 Act_Hold;
 Val_Descuento;
...
procedure TfrmEntradaLaboratorio.ImpresionRecibo;
begin
  DM.qrCobro.Close;
...
procedure TfrmEntradaLaboratorio.ImpresionNC;
begin
  DM.qrCobro.Close;
  DM.qrCobro.Parameters[0].Value := cobroid;
  DM.qrCobro.Open;
...
procedure TfrmEntradaLaboratorio.CambiarLaboratorioNoenlasLineas1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.CancelarCobro;
begin
...
function TfrmEntradaLaboratorio.GetMuestraNo : String;
Var
 qsecdoc : TADOQuery;
 anio, mes, dia : Word;
begin
...
function TfrmEntradaLaboratorio.GetReciboNo : String;
begin
  result := GetNextSecDoc('RI');
end;


function TfrmEntradaLaboratorio.GetNotaCredito : String;
begin
  result := GetNextSecDoc('NC');
end;


procedure TfrmEntradaLaboratorio.ActDetalle;
var
 bM : TBookMark;
 qact : TADOQuery;
begin
 ...
 begin
  ...
  qact.Close;
  qact.SQL.Clear;
  qact.SQL.Text := ' UPDATE BSVENTADETALLE SET '+
...
function TfrmEntradaLaboratorio.ImporteBrutoExclLineaActual(refrecid : integer ;secuencia : integer) : Double;
var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
 qfind.Close;
 qfind.SQL.Text := 'Select sum(Precio) from BSVENTADETALLE Where RefRecid = :rec and Secuencia <> :sec';
...
function TfrmEntradaLaboratorio.ImporteBrutoExclLineaActualAndCobertura(refrecid : integer ;secuencia : integer) : Double;
var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
function TfrmEntradaLaboratorio.TotalDescuentoLinea : Double;
var
...
function TfrmEntradaLaboratorio.ImporteLinea : Double;
Var
...
function TfrmEntradaLaboratorio.CoberturaAplicada : Double;
var
...
procedure TfrmEntradaLaboratorio.CodigodeBarra1Click(Sender: TObject);
begin
  inherited;
  Showmessage('Pendiente de Toma de Muestra.!!!');
end;

procedure TfrmEntradaLaboratorio.RegistrarCobrosTemporales1Click(Sender: TObject);
begin
  inherited;
  frmMain.LanzaVentana(-8024);
end;

procedure TfrmEntradaLaboratorio.RegistrarFechaPrometida1Click(Sender: TObject);
begin
  inherited;
  Showmessage('Pendiente de Toma de Muestra.!!!');
end;

procedure TfrmEntradaLaboratorio.ConfirmarCobertura1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.btcancelarClick(Sender: TObject);
begin
  inherited;
    ...
    if DM.Mensaje('Existe una entrada en proceso. Desea cancelar la entrada?',mb_yesno) = id_yes then
     begin
      if qrEntradaPacienteEntradaID.Value = '' then
       begin
        DM.DataBase.Execute('Delete from BSVENTADETALLE Where Refrecid = '+qrEntradaPacienteRecid.AsString);
...
procedure TfrmEntradaLaboratorio.btclienteClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.bteliminarClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.btentradaClick(Sender: TObject);
var
 continuar : Boolean;
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.btgrabarClick(Sender: TObject);
var
...
//        Mirar_Valor_CAB;
     ...
     If not (qrEntradaPacienteDetalle.State in [dsEdit, dsInsert]) then
       Begin
...
//     If qrEntradaPaciente.State in [dsInsert] Then
     ...
     //DMB.PasarVenta(qrEntradaPacienteMuestraNo.Value);
...
procedure TfrmEntradaLaboratorio.BtHoldClick(Sender: TObject);
begin
 inherited;
  if ValidateRecord(dsEntradaPaciente.DataSet) and ValidateCliente then
   begin
...
procedure TfrmEntradaLaboratorio.btimpcotClick(Sender: TObject);
begin
  inherited;
  if qrEntradaPacienteEntradaID.Value <> '' then
  begin
...
procedure TfrmEntradaLaboratorio.btnotacreditoClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.btpacienteClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.btpruebaClick(Sender: TObject);
var
  PruebaId: String;
...
begin
  ...
  PruebaId:= DMB.BuscarProducto;

  if (trim(PruebaId) = '') then Exit;


  qrEntradaPacienteDetalle.Insert;
  qrEntradaPacienteDetallePruebaID.Value := PruebaId;
  ...
  qrDatosProducto.Parameters.ParamByName('CodigoId').Value:= PruebaId;
  qrDatosProducto.Open;

  qrEntradaPacienteDetalleDESCRIPCION.Value:= qrDatosProductoProductoDes.Value;
  qrEntradaPacienteDetallePRECIO.Value:= qrDatosProductoPrecio.Value;
  qrEntradaPacienteDetalleTOTALLINEA.Value:= qrDatosProductoPrecio.Value;
...
procedure TfrmEntradaLaboratorio.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.FormDestroy(Sender: TObject);
begin
  if (qrEntradaPacienteEntradaID.asstring = '') then
    begin
      DM.DataBase.Execute('Delete from PTGastosVarioTrans Where Refrecid = '+qrEntradaPacienteRecid.AsString);
...
procedure TfrmEntradaLaboratorio.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.SetColorTo(iColor: TColor);
var
  i : Integer;
  oColor : TColor;
begin
...
procedure TfrmEntradaLaboratorio.SetReadOnlyToNoEdit;
var
  i : Integer;
begin
  for i := 0 to ComponentCount - 1 do begin
...
procedure TfrmEntradaLaboratorio.SetComponentReadOnly(thisComponent: TComponent; bFlag : Boolean);
var
  i : Integer;
  oColor : TColor;
begin
...
procedure TfrmEntradaLaboratorio.UnSetReadOnlyToNoEdit;
var
  i : Integer;
begin
  for i := 0 to ComponentCount - 1 do begin
...
procedure TfrmEntradaLaboratorio.NotadeCredito1Click(Sender: TObject);
begin
  inherited;
  frmMain.LanzaConsulta(-7991, qrEntradaPacienteRecid.asstring);
end;

procedure TfrmEntradaLaboratorio.NuevaVenta;
begin
  Operacion := 'E';
  cobroid := '';
  DM.qrParametro.Close;
  ...
  qrEntradaPacienteDATAAREAID.Value := 'bds';
 ...
 // SetReadOnlyToNoEdit;
  ...
  edtipocliente.ItemIndex:= 0;
end;


procedure TfrmEntradaLaboratorio.qrEntradaPacienteAfterPost(DataSet: TDataSet);
var
 qdelete : TADOQuery;
 tipodoc : string;
begin
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteCalcFields(DataSet: TDataSet);
begin
  inherited;
  qrEntradaPacienteTotalPendiente.Value := qrEntradaPacienteNeto.Value - qrEntradaPacienteTotalPagado.Value;
end;

procedure TfrmEntradaLaboratorio.qrEntradaPacienteClienteIDChange(Sender: TField);
begin
  inherited;
  If qrEntradaPaciente.State in [dsEdit,dsInsert] then
    ActTotales;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteCoberturaExpPorcValidate(
  Sender: TField);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteCoberturaPorcValidate(Sender: TField);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteCoberturaValorValidate(
  Sender: TField);
begin
  inherited;
{
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleAfterDelete(DataSet: TDataSet);
begin
  inherited;
  ActTotales;
end;

procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleAfterPost(DataSet: TDataSet);
begin
  inherited;
  if qrEntradaPaciente.State in [dsEdit, dsInsert] then
    Begin
...
procedure TfrmEntradaLaboratorio.VerificarPagoDiferencia;
var
 qfind : TADOQuery;
begin
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleBeforeDelete(
  DataSet: TDataSet);
begin
  inherited;
  if ( qrEntradaPacienteDetalleCoberturaAplica.Value = 1) then
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleBeforeEdit(DataSet: TDataSet);
begin
  inherited;
  comboprecio := True;
end;

procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleBeforeInsert(DataSet: TDataSet);
begin
  inherited;
  comboprecio := True;
end;

procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleBeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleDescuentoChange(
  Sender: TField);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetalleNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetallePruebaIDChange(Sender: TField);
var
 qfind : TADOQuery;
begin
  inherited;
  ...
  begin
   ...
   If (qrEntradaPacienteFactExterior.Value = 1) then
     Begin
       ...
       qrEntradaPacienteDetallePrecio.Value := qfindprueba.FieldByName('Precio').AsCurrency;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDetallePruebaIDValidate(Sender: TField);
begin
  inherited;
    Begin
      qfindprueba := DM.find('Select * from ProductosComoPruebas where PruebaID = :pru',qrEntradaPacienteDetallePruebaID.value);
      Exit; //No validar
...
//          if (qrEntradaPacienteRepMuestra.Value = True And Act_2da = True) Then
          if (Act_2da = True) Then
            Begin
              qfindRepetir := DM.NewQuery;
              qfindRepetir.Close;
              qfindRepetir.SQL.Text := ' SELECT * FROM BSVENTADETALLE '+
                                       ' WHERE RefRecId = :rec '+
                                       ' AND MuestraNo = :num '+
                                       ' AND PruebaId = :pru ';
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteDoctorIDChange(Sender: TField);
begin
  inherited;
//  Buscar_Doctor;
end;

procedure TfrmEntradaLaboratorio.EdDoctorPropertiesChange(Sender: TObject);
begin
  inherited;
  Buscar_Doctor;
end;

procedure TfrmEntradaLaboratorio.Buscar_Doctor;
var
 qfind : TADOQuery;
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteNombrePacienteValidate(
  Sender: TField);
begin
  inherited;
  if (qrEntradaPacienteOrigen.Value = DM.qrParametroGrupoLab.Value) then
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteOrigenChange(Sender: TField);
begin
  inherited;
  actinterface;
end;

procedure TfrmEntradaLaboratorio.qrEntradaPacienteTipoDocumentoChange(Sender: TField);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.qrEntradaPacienteTipoEntradaChange(Sender: TField);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.Materiales1Click(Sender: TObject);
begin
  inherited;
  frmMain.LanzaConsulta(-7982,qrEntradaPacienteRecid.AsString);
end;

function TfrmEntradaLaboratorio.MaxSecuenciaLinea : Integer;
Var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
 ...
 qfind.Close;
 qfind.SQL.Text := 'Select max(Secuencia) from BSVENTADETALLE Where refRecid = '+ inttostr(recid);
 qfind.Open;
...
function TfrmEntradaLaboratorio.Buscar_Linea: Integer;
Var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
procedure TfrmEntradaLaboratorio.MenuItem1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.ActTotales;
Var
 qfind : TADOQuery;
begin
 ActDetalle;
...
procedure TfrmEntradaLaboratorio.AsignarDescuentoAutorizadoporCobros1Click(
  Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.AsignarDescuentoPermitidoalUsuario1Click(
  Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.RecalcularLineas;
begin
...
procedure TfrmEntradaLaboratorio.RecibodeCobro1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.EliminarDescuentoAplicado1Click(Sender: TObject);
begin
  inherited;
  qrEntradaPacienteDescuentoPorc.Value := 0;
  qrEntradaPacienteDescuentoValor.Value := 0;
...
procedure TfrmEntradaLaboratorio.EliminarOrdenesenHold1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.MarcarPruebaNOProcesar1Click(Sender: TObject);
begin
  inherited;
  No_Procesar;
end;

function TfrmEntradaLaboratorio.ExisteCobro : Boolean;
Var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
function TfrmEntradaLaboratorio.TipoCobro : String;
Var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
...
procedure TfrmEntradaLaboratorio.DesabilitarBotones(_SiNo : Boolean);
begin
...
procedure TfrmEntradaLaboratorio.EdCoberturaValorExit(Sender: TObject);
begin
  inherited;
  Act_Cobertura
end;
...
procedure TfrmEntradaLaboratorio.Act_Cobertura;
begin
 if qrEntradaPaciente.State in [dsedit, dsinsert] then
  begin
   qrEntradaPacienteDescuentoValor.Value := 0;
...
function TfrmEntradaLaboratorio.GetNextSecDoc(tipodoc : string) : String;
var
 qsecdoc : TADOQuery;
begin
 qsecdoc := DM.NewQuery;
...
procedure TfrmEntradaLaboratorio.GastosVarios1Click(Sender: TObject);
begin
  inherited;
  frmMain.lanzaConsulta(-7984, qrEntradaPacienteRecid.asstring);
end;

function TfrmEntradaLaboratorio.GetEntradaID : String;
begin
 DM.qrParametro.Close;
 DM.qrParametro.Open;
...
procedure TfrmEntradaLaboratorio.Boton_Cobrar;
begin
...
procedure TfrmEntradaLaboratorio.Viejo_Cobrar;
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.CrearCobro;
Var
 qinsert    : TADOQuery;
...
procedure TfrmEntradaLaboratorio.RefrescarCobro;
Var
 qfind : TADOQuery;
begin
...
procedure TfrmEntradaLaboratorio.ActCobro;
Var
 qupdate    : TADOquery;
 qfind      : TADOQuery;
...
Function TfrmEntradaLaboratorio.buscar_valores;
Var
  valor : Boolean;
begin
  valor := True;
...
procedure TfrmEntradaLaboratorio.correccion;
begin
  mdPago.First;
  While Not mdPago.Eof do
   Begin
...
function TfrmEntradaLaboratorio.ValidarMontoCobrado : Boolean;
Var
 monto : Double;
begin
 monto := 0;
...
procedure TfrmEntradaLaboratorio.CrearNotaCredito;
Var
 qinsert : TADOQuery;
begin
 DM.qrParametro.Close;
...
procedure TfrmEntradaLaboratorio.ActNotaCredito;
Var
 qupdate : TADOquery;
 qfind : TADOQuery;
 qfindcobro : TADoQuery;
...
procedure TfrmEntradaLaboratorio.CrearNotaDebito;
Var
 qinsert : TADOQuery;
begin
 DM.qrParametro.Close;
...
procedure TfrmEntradaLaboratorio.ActNotaDebito;
Var
 qupdate : TADOquery;
 qfind : TADOQuery;
 qfindcobro : TADoQuery;
...
procedure TfrmEntradaLaboratorio.VolantedePruebasARS1Click(Sender: TObject);
begin
  inherited;
  if qrEntradaPacienteEntradaID.Value <> '' then
  begin
...
procedure TfrmEntradaLaboratorio.VolantedePruebasTomaMuestra1Click(Sender: TObject);
begin
  inherited;
  if qrEntradaPacienteEntradaID.Value <> '' then
  begin
...
procedure TfrmEntradaLaboratorio.FacturaExterior1Click(Sender: TObject);
begin
 inherited;
 if qrEntradaPacienteEntradaID.Value <> '' then
   begin
...
procedure TfrmEntradaLaboratorio.BuscarDatos;
begin
  qrEntradaPaciente.Close;
  qrEntradaPaciente.parameters[0].Value := recid;
  qrEntradaPaciente.Open;
...
procedure TfrmEntradaLaboratorio.RefrescarInterface;
begin
 actinterface;
...
//Validando Los Parametros de Descuentos.
procedure TfrmEntradaLaboratorio.Val_Descuento;
begin
 //Buscando los Datos del Cliente a Facturar y Sus Parametros.
 Buscar_Cliente;
{
...
function TfrmEntradaLaboratorio.CanCambiarLaboratorio;
var
 qfind : TADOQuery;
begin
...
function TfrmEntradaLaboratorio.Asignar_Descuento;
var
 qfind : TADOQuery;
begin
 qfind := DM.Find('SELECT * FROM SysUsuarios WHERE UserID = :user',DM.CurUser);
...
procedure TfrmEntradaLaboratorio.RegistrarPruebadelExterior1Click(Sender: TObject);
begin
 inherited;
  DM.qrParametro.Close;
  DM.qrParametro.Open;
...
procedure TfrmEntradaLaboratorio.Act_Hold;
var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
 ...
 qfind.Close;
 qfind.SQL.Text := 'SELECT COUNT(RecId) FROM BSVenta '+
...
//Eliminando Los Documentos en Hold.
procedure TfrmEntradaLaboratorio.Del_Hold;
var
 qfind : TADOQuery;
begin
 qfind := DM.NewQuery;
 ...
 qfind.Close;
 qfind.SQL.Text := ' SELECT * FROM BSVenta '+
 ...
 If (qfind.RecordCount > 0) Then
   Begin
     ...
     While Not qfind.Eof do
      Begin
        DM.DataBase.Execute('Delete from BSVenta Where RecId = '+ qfind.FieldByName('RecId').AsString);
        DM.DataBase.Execute('Delete from BSVENTADETALLE Where Refrecid = '+ qfind.FieldByName('RecId').AsString);
...
procedure TfrmEntradaLaboratorio.No_Procesar;
Var
 qact : TADOQuery;
begin
{
...
procedure TfrmEntradaLaboratorio.Asignar2daMuestra1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.Buscar_Muestra;
Var
 qfind  : TADOQuery;
 Dias   : Extended;
begin
...
procedure TfrmEntradaLaboratorio.Buscar_Combo;
begin
...
procedure TfrmEntradaLaboratorio.Insertar_Combo_Precio;
begin
...
procedure TfrmEntradaLaboratorio.Insertar_Combo_Blanco;
begin
 While Not qfindcombo.Eof Do
  Begin
   If (qrEntradaPacienteDetallePruebaID.Value = '') Or
...
//Actualizando los Datos del Cliente
procedure TfrmEntradaLaboratorio.Act_Cliente;
Var
...
//Buscando los Datos del Cliente a Facturar y Sus Parametros.
procedure TfrmEntradaLaboratorio.Buscar_Cliente;
begin
// If (qrEntradaPacienteClienteId.Value <> '') then
//  Begin
...
//Buscando los Datos del PACIENTE Registrado y Sus Parametros.
procedure TfrmEntradaLaboratorio.Buscar_Paciente;
begin
...
//Buscando los Datos del Grupo del Cliente.
Procedure TfrmEntradaLaboratorio.Buscar_Grupo_Clt(Grupo: string);
begin
...
//Buscando los Datos del Grupo del Paciente.
Procedure TfrmEntradaLaboratorio.Buscar_Grupo_Pac(Grupo: string);
begin
...
procedure TfrmEntradaLaboratorio.Buscar_Cod_Cups(cliente: String; prueba: String);
begin
...
//Insertando Los Datos de la Tarjeta de Descuento.
procedure TfrmEntradaLaboratorio.Insert_Card_Descuento;
...
procedure TfrmEntradaLaboratorio.ppHeaderBand3BeforePrint(Sender: TObject);
begin
...
procedure TfrmEntradaLaboratorio.ppSummaryFacturaSeguroBeforePrint(Sender: TObject);
begin
...
procedure TfrmEntradaLaboratorio.ppSummaryBand4BeforePrint(Sender: TObject);
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.ppSummaryFactPacienteConSeguroBeforePrint(Sender: TObject);
begin
  inherited;
  Buscar_Cliente;
...
procedure TfrmEntradaLaboratorio.ppSummaryFactPacienteSinSeguroBeforePrint(Sender: TObject);
begin
  //Buscando los Datos del Cliente.
{  Buscar_Cliente;
...
procedure TfrmEntradaLaboratorio.ppDetailBand3BeforePrint(Sender: TObject);
begin
...
procedure TfrmEntradaLaboratorio.ppDetailBand4BeforePrint(Sender: TObject);
begin
...
procedure TfrmEntradaLaboratorio.ppDetailBandSinSeguroBeforePrint(Sender: TObject);
begin
  {qfindprint := DM.find('SELECT * FROM PTPrueba WHERE PruebaID = :pru', qrEntradaPacienteDetallePruebaId.Value);
...
procedure TfrmEntradaLaboratorio.InsertarMaterialesDefault;
Var
...
procedure TfrmEntradaLaboratorio.InsertarMaterialesPrueba;
Var
 qfind, qinsert : TADOQuery;
begin
 qfind := DM.NewQuery;
...
function TfrmEntradaLaboratorio.Buscar_Labels(muestra: String; prueba: String): Boolean;
Var
 qfind : TADOQuery;
 valor : Boolean;
begin
...
procedure TfrmEntradaLaboratorio.CrearLabels(SecAct: Integer);
var
...
begin
 ...
 qfindLb.SQL.Text := ' SELECT a.Entradaid, a.pacienteid, a.sucursalid, b.pruebaid, c.Descripcion, '+
                     ...
                     ' FROM BSVenta a, BSVENTADETALLE b, ptPrueba c '+
...
procedure TfrmEntradaLaboratorio.InsertarLabel;
var
 Qinsert    : TADOQuery;
 labelrecid : Integer;
begin
...
procedure TfrmEntradaLaboratorio.ImprimirLabel;
var
 qfind  : TADOQuery;
 texto  : TextFile;
 cadena : String;
...
//Calculando el Nuevo Precio de la Prueba Dependiendo del Cliente Actual.
procedure TfrmEntradaLaboratorio.Confirmar_Precio_Prueba;
var
 qprueba : TADOQuery;
begin
  inherited;
...
procedure TfrmEntradaLaboratorio.Procesar_Usuario;
Var
  Valor : Boolean;
  Pass2, Clave : Integer;
...
Function TfrmEntradaLaboratorio.Crear_Clave(cedularnc : String) : Integer;
Var
  Numero  : Double;
  Valor   : Int64;
//  Valor   : Integer;
...
//Creando el Usuario de Internet.
Function TfrmEntradaLaboratorio.Crear_Usuario(nombre : String) : String;
Var
  Texto : String;
...
Function TfrmEntradaLaboratorio.Valida_Usuario(usuario : String) : Boolean;
Var
 qfind  : TAdoQuery;
begin
...
procedure TfrmEntradaLaboratorio.Insertar_Usuario(usuario: string; clave: string; name: string; cedula: string;
...
//Validando la Existencia del Usuario Creado.
Function TfrmEntradaLaboratorio.Buscar_User_Internet(Cliente : String) : Boolean;
Var
 qfind  : TAdoQuery;
begin
...
function TfrmEntradaLaboratorio.BuscarDia(dia: Integer) : Integer;
Var
 valor : Integer;
begin
 valor := 0;
...
function TfrmEntradaLaboratorio.FechaPrometida(Prueba: string) : TDate;
Var
 fecha : TDate;
 qtest : TADOQuery;
 qfrec : TADOQuery;
...
procedure TfrmEntradaLaboratorio.Agregar_Pacientes(PacienteId : String);
Var
 sqlString : String;
...
Function TfrmEntradaLaboratorio.Cargar_Valor(campo: String; valor: Variant): String;
begin
 Result := campo + ' - ( ' + VarToStr(valor) + ' ).';
end;


procedure TfrmEntradaLaboratorio.Mirar_Valor_CAB;
Var idx : Integer;
begin
...
procedure TfrmEntradaLaboratorio.Mirar_Valor_DET;
Var idx : Integer;
begin
