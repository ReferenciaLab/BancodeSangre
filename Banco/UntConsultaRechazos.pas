...
procedure TFrmconsultaRechazos.cxButton2Click(Sender: TObject);
begin
  inherited;
try
 With dm.Qutilitario do
  begin
    Close;
    Sql.Clear;
    Sql.Add('Update BSDonacion set FechaAuditoria =:f, UsuarioAudito =:U, NotaAuditoria =:n ');
    Parameters.ParamByName('f').Value := cxDateEdit1.Date;
    Parameters.ParamByName('u').Value :=  DM.CurUser;
    Parameters.ParamByName('n').Value := cxMemo1.Text;
    Sql.Add('where DonacionID =' + #39 + QdatosDonacionID.Value + #39);
    ExecSQL;
  end;
