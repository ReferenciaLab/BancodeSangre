   ...
   Begin
     ...
     While Not qrCombos.Eof Do
      Begin
        ...
        if (qrprueba.FieldByName('Orina').AsBoolean = True) then
          orina := True;

        if (qrprueba.FieldByName('Sangre').AsBoolean = True) then
          sangre := True;
...
// OJO - MIENTRAS TANTO.            seleccion := seleccion +
...
////////////////////////////////////////////////////////////////////////////////
  ...
  DM.Info('La Exportacion en el Formato Viejo Fue Concluida Exitosamente.!!!');
end;
...
// OJO - Campo Para Indicar Si Se Tomara Una Muestra Sanguinea.     _sangre     :=  Copy(valor, 102, 1);
// OJO - Campo Para Indicar Si Se Tomara Una Muestra de Orina.     _orina      :=  Copy(valor, 103, 1);
     ...
     If (_nail = '1') Then
       Begin
         ...
         qact.SQL.Text := ' UPDATE PTClientesPlanes SET '+
                          ...
                          ' WHERE PlanID = :plan '+
// OJO - BLOQUEADO PARA LA DEMOSTRACION.                          ' AND MuestraNo = :mues '+
                          ' AND EmpleadoId = :empl ';
...
// OJO - BLOQUEADO PARA LA DEMOSTRACION.         qact.Parameters.ParamByName('mues').Value := _muestraNo;
